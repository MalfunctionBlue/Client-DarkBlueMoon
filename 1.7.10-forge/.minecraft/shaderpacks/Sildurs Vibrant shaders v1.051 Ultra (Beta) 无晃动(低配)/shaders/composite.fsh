...
const float shadowDistance = 128.0f;
	#define SHADOW_DARKNESS 0.65				//shadow darkness levels, lower values mean darker shadows, see .vsh for colors /0.25 is default
	//#define MAC_LAG_FIX						//If you have very low fps "5-6" enable this and disable #HQ_SHADOW_FILTER	
	#define HQ_SHADOW_FILTER					//Disable this if using MAC_LAG_FIX	
...
#define GODRAYS
		...
		const int NUM_SAMPLES = 6;				//increase this for better quality at the cost of performance /5 is default
...
float EarlySunrise = ((clamp(timefract, 23000.0, 24000.0) - 23000.0) / 1000.0) + (1.0 - (clamp(timefract, 0.0, 1000.0)/1000.0));
float EarlySunrise2 = ((clamp(timefract, 0.0, 1000.0)) / 1000.0) - ((clamp(timefract, 0.0, 1500.0)/1500.0));
float EarlySunrise3 = ((clamp(timefract, 0.0, 1500.0)) / 1500.0) - ((clamp(timefract, 0.0, 2000.0)/2000.0));

float LateSunrise  = ((clamp(timefract, 0.0, 2000.0)) / 2000.0) - ((clamp(timefract, 0.0, 2500.0)/2500.0));
float LateSunrise2  = ((clamp(timefract, 0.0, 2500.0)) / 2500.0) - ((clamp(timefract, 0.0, 3000.0)/3000.0));
float LateSunrise3  = ((clamp(timefract, 0.0, 3000.0)) / 3000.0) - ((clamp(timefract, 0.0, 4000.0)/4000.0));
...
float torch_lightmap = pow(max(aux.b-2/16.0,0.0),4.0)*4.0;
...
vec3 getSkyColor(inout vec3 CalcClouds) {
//sky gradient
/*----------*/
vec3 sky_color = pow(ambient_color,vec3(2.2))*2.0;
vec3 sVector = normalize(CalcClouds);
vec3 upVector = normalize(upPosition);
...
sky_color = mix(sky_color,pow(sunlight_color,vec3(2.2)),1-exp(-0.3*L*(1-rainStrength*0.8)));
...
//cloud generation
/*----------*/
vec3 tpos = vec3(gbufferModelViewInverse * vec4(CalcClouds,1.0));
...
//float canHit = length(intersection)-length(tpos);
	...
	float c = max(noise-1.0-coverageVariance*0.1+rainStrength*0.18,0.0);
	  ...
	  float N = 12.0;
vec3 cloud_color = skyLightIntegral(sVector)/1.57 + sunlight_color*48.0*pow(max(cosY,0.0),N)*(N+1)/6.28 * (cloud*0.5+0.5) * (1-rainStrength);	//coloring clouds
/*----------*/
	...
	if (land > 0.9 && isEyeInWater < 0.1) {
		...
		if (comparedepth > 0.0f &&	worldposition.s < 1.0f && worldposition.s > 0.0f && worldposition.t < 1.0f && worldposition.t > 0.0f) {
			...
			step = 1.0/shadowMapResolution*(1.0+rainStrength*5.0);
				...
				shading += shadow2D(shadow,vec3(worldposition.st + vec2(step,step), worldposition.z-diffthresh/300)).x;
				shading += shadow2D(shadow,vec3(worldposition.st + vec2(step,-step), worldposition.z-diffthresh/300)).x;
				shading += shadow2D(shadow,vec3(worldposition.st + vec2(-step,-step), worldposition.z-diffthresh/300)).x;
				shading += shadow2D(shadow,vec3(worldposition.st + vec2(-step,step), worldposition.z-diffthresh/300)).x;
				shading = shading/5.0;
...
if (EarlySunrise2 > 0.1){
SunStrength = 14.0;
} else 
if (EarlySunrise3 > 0.1){
SunStrength = 12.0;
} else
if (LateSunrise > 0.1){
SunStrength = 10.0;
...
SunStrength = 4.0;
		...
		float sss = subSurfaceScattering(fragposition.xyz,30.0)*translucent*SunStrength*2.0;
		sss = mix(0.0,sss,max(shadow_fade-0.1,0.0)*1.111)*0.5+0.5;
		shading = clamp(shading,0.0,1.0);
		float handLight = (handlight*3.0)/pow(1.0+length(fragposition.xyz/2.0),2.0)*sqrt(dot(normalize(fragposition.xyz), -normal)*0.5+0.5);
		
	//Apply different lightmaps to image
		vec3 Sunlight_lightmap = mix(sunlight_color,vec3(0.5),rainStrength*0.9)*mix(max(sky_lightmap-rainStrength*0.95,0.0),shading*(1.0-rainStrength*0.95),shadow_fade)*SunStrength *sunlight_direct*transition_fading ;
		/*
		...
		*/
		...
		float torchbalance = 1.0f+(aux.b,0.12)*80.0;
		vec3 torchcolor = vec3(1.23, 0.22, 0.11)*torchbalance;
	...
	//Add all light elements together
		color = (bounceSunlight*bounceSunlight*sky_lightmap*0.00+sky_light + 0.0*ao + Sunlight_lightmap + color_torchlight*ao  +  sss * sunlight_color * shading *(1.0-rainStrength*0.9)*transition_fading)*color;
