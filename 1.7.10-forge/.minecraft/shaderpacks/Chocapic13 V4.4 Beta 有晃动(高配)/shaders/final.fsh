...
#define BLOOM								//do the "fog blur" in the same time
	...
	//#define B_LQ			
	#define B_TRESH 0.2				
	#define B_RAD 15.0					//sampling circle size multiplier, don't affect performance
	#define B_INTENSITY 2.0 //basic multiplier

//#define DOF
#define HEXAGONAL_BOKEH
//#define CIRCULAR_BOKEH
	//lens properties
...
*/
	...
	vec3 bcolor = color/16.0;
	float nb = 0.0;
	vec2 bcoord = vec2(0.0);
	#ifdef HEXAGONAL_BOKEH
if (pcoc > pw*1.5) {
	for ( int i = 0; i < 60; i++) {
		sample = texture2D(gaux2, texcoord.xy + hex_offsets[i]*pw*10.0*vec2(1.0,aspectRatio));
		bcolor += sample.rgb*sample.a;
	}
	#else
	if (pcoc > pw*1.5) {
	for ( int i = 0; i < 60; i++) {
		sample = texture2D(gaux2, texcoord.xy + offsets[i]*pcoc*vec2(1.0,aspectRatio));
		bcolor += sample.rgb;
	}
	#endif
	color.rgb = bcolor/61.0*16.0;
	}
#endif
...
/*
			...
			vec3 csample = texLodb(gaux2,texcoord.xy).rgb;

			
		
		//fog blurring with the distance
		color.rgb = mix(color,blur/25.0,depth_diff*isEyeInWater*0.4+fog*(1.0-isEyeInWater)*rainStrength*0.8);
		color = color*0.5+csample*B_INTENSITY/2.0;
	#endif
*/
	#ifdef B_HQ
			float scale = length(vec2(pw,ph));
			vec3 csample = vec3(0.0);
				...
				float weight = exp(-length(offsets[i]));
				csample += pow(texture2D(gaux2,texcoord.xy + coords*B_RAD*scale*vec2(1.0,aspectRatio)).rgb*weight,vec3(2.2))*16.0 ;
				//blur += sample * (length(coords))/2.0;
			}
			
		
		//fog blurring with the distance
		//color.rgb = mix(color,blur/60.0*16.0,fog*(1.0-isEyeInWater)*(rainStrength*0.0+1.0));
		color += max(csample/60.0*B_INTENSITY,0.0);
	#endif
...
color += sunlight * vec3(0.0,0.0,1.0)*anamorphic_lens*LENS_STRENGTH*sunvisibility*truepos*4.0;
}
}
...
color += 1/16.0*fogclr*rainlens*(eyeBrightness.y/255.0);
...
/*
	...
	vec3 value = pow(color.rgb, vec3(contrast)) / (pow(color.rgb, vec3(contrast)) + pow(IAverage, vec3(contrast)))*2.0;
...
//vec3 whiteScale = 1.0f/Uncharted2Tonemap((1.0-vec3(log(clamp(avglight*40.0,0.01,0.97)))/log(0.005))*16.0);
...
//color = abs(curr*whiteScale-curr*1.0f/Uncharted2Tonemap(vec3(W)));
color = pow(color,vec3(1.0/2.2));


	//if (texcoord.x < 0.1 && texcoord.y < 0.1) color.rgb = vec3(texture2D(gaux2,vec2(1.0)).a)*5.0;
