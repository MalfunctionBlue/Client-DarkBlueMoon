...
#define SHADOW_DARKNESS 0.8	//shadow darkness levels, lower values mean darker shadows, see .vsh for colors /0.25 is default
...
//----------Lighting----------//
	...
	#define SUNLIGHTAMOUNT 7.0		//change sunlight strength , see .vsh for colors. /1.7 is default
	
	//Torch Color//
	vec3 torchcolor = vec3(0.6,0.32,0.1);		//RGB - Red, Green, Blue / vec3(0.6,0.32,0.1) is default
	#define TORCH_ATTEN 5.0						//how much the torch light will be attenuated (decrease if you want that the torches cover a bigger area))/3.0 is default
	#define TORCH_INTENSITY 8.0					//torch light intensity /2.6 is default
...
float fresnel = clamp(pow(1.0 + normalDotEye, fpow),0.0,1.0);

float pi = 3.1415927;
float n =  pow(2.0,pow(gloss,1.0/2.2)*8.0);
return 1.0-exp(-(pow(blinnTerm, n )*((n+8.0)/(8*pi)))*fresnel*gloss*visibility*2.0);
}
...
void main() {
	...
	shadow_fade = 1.0-clamp((max(abs(worldposition.x-0.5),abs(worldposition.y-0.5))*2.0-0.9),0.0,0.1)*10.0;
		if (worldposition.x > 0.0 && worldposition.y > 0.0 && worldposition.x < 1.0 && worldposition.x < 1.0) {
			...
			if (shadowexit > 0.1) {
				...
				for(int i = 0; i < 25; i++){
					weight = exp(-length(circle_offsets[i])*step);
					shading += shadow2D(shadow,vec3(worldposition.st + circle_offsets[i]*step, worldposition.z-diffthresh/300)).x*weight;
					totalweight += weight;
				}
				shading /= totalweight;
			#endif
	...
	spec =  1.0-exp(-ctorspec(fragposition.xyz,lightVector,normalize(normal),0.02,fresnel_pow) *land * (1.0-isEyeInWater) * shading * (1.0-night*0.75)*gfactor);
	}
